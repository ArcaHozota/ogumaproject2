...
public class EmployeeRepositoryImpl implements EmployeeRepository {
	...
	public Integer countByKeyword(final String keyword) {
		return this.jdbcClient.sql(
				"SELECT COUNT(1) FROM PPOG_EMPLOYEES_VIEW PEV WHERE PEV.LOGIN_ACCOUNT LIKE ? OR PEV.USERNAME LIKE ? OR PEV.EMAIL LIKE ?")
				.params(keyword, keyword, keyword).query(Integer.class).single();
	}

	@Override
	public Integer countByName(final String name) {
		return this.jdbcClient.sql("SELECT COUNT(1) FROM PPOG_EMPLOYEES PE WHERE PE.LOGIN_ACCOUNT = ?").param(name)
	...
	public List<Employee> getListByForeignKey(final Long foreignKey) {
		return this.jdbcClient.sql(
				"SELECT PEV.* FROM PPOG_EMPLOYEES_VIEW PEV INNER JOIN PPOG_EMPLOYEES_ROLE_VIEW PERV ON PERV.EMPLOYEE_ID = PEV.ID　"
						+ "WHERE PERV.ROLE_ID = ?")
	...
	public List<Employee> getListByIds(final List<Long> ids) {
		return this.jdbcClient.sql("SELECT PEV.* FROM PPOG_EMPLOYEES_VIEW PEV WHERE PEV.ID IN (?)").params(ids)
	...
	public Employee getOneById(final Long id) {
		return this.jdbcClient.sql("SELECT PEV.* FROM PPOG_EMPLOYEES_VIEW PEV WHERE PEV.ID = ?").param(id)
	...
	public Employee getOneByLoginAccount(final String loginAccount) {
		return this.jdbcClient
				.sql("SELECT PEV.* FROM PPOG_EMPLOYEES_VIEW PEV WHERE PEV.LOGIN_ACCOUNT = ? OR PEV.EMAIL = ?")
	...
	public List<Employee> pagination(final Integer offset, final Integer pageSize, final String keyword) {
		return this.jdbcClient
				.sql("SELECT PEV.* FROM PPOG_EMPLOYEES_VIEW PEV WHERE PEV.LOGIN_ACCOUNT LIKE ? OR PEV.USERNAME LIKE ? "
