...
(function(p) {
			...
			function(b, a, c, d, f, g) {
				if (c) {
					...
					c[e] && c[e].length > 0
						...
						: ((c.isParent =
							typeof c.isParent == "string"
								? j.eqs(c.isParent, "true")
								: !!c.isParent),
		...
		h = {
			...
			getNextNode: function(b, a) {
				...
				for (
					...
					d = a.parentTId ? a.getParentNode() : h.getRoot(b),
					f = 0,
					g = d[c].length - 1;
					f <= g;
					f++
			...
			getNodeTitle: function(b, a) {
				return (
					"" + a[b.data.key.title === "" ? b.data.key.name : b.data.key.title]
				);
			},
			getNodes: function(b) {
			...
			getNodesByParam: function(b, a, c, d) {
				if (!a || !c) return [];
				for (
					var f = b.data.key.children, g = [], l = 0, e = a.length;
					l < e;
					l++
				)
					a[l][c] == d && g.push(a[l]),
			...
			getNodesByFilter: function(b, a, c, d, f) {
				if (!a) return d ? null : [];
				for (
					var g = b.data.key.children, e = d ? null : [], i = 0, o = a.length;
					i < o;
					i++
				) {
			...
			isSelectedNode: function(b, a) {
				for (
					var c = h.getRoot(b), d = 0, f = c.curSelectedList.length;
					d < f;
					d++
				)
	...
	H = function(b, a) {
		var c = h.getSetting(b.data.treeId),
			d =
				c.view.autoCancelSelected &&
					(b.ctrlKey || b.metaKey) &&
					h.isSelectedNode(c, a)
					? 0
					: c.view.autoCancelSelected &&
						(b.ctrlKey || b.metaKey) &&
						c.view.selectedMulti
						? 2
						: 1;
	...
	var j = {
		...
		$: function(b, a, c) {
			...
			return typeof b == "string"
				? p(b, c ? c.treeObj.get(0).ownerDocument : null)
				: p("#" + b.tId + a, c ? c.treeObj : null);
		},
		...
		getNodeMainDom: function(b) {
			return (
				p(b).parent("li").get(0) || p(b).parentsUntil("li").parent().get(0)
			);
		},
		...
		i = {
			...
			appendNodes: function(b, a, c, d, f, g) {
				if (!c) return [];
				for (
					var e = [], j = b.data.key.children, k = 0, m = c.length;
					k < m;
					k++
				) {
					var n = c[k];
					...
					n[j] &&
						n[j].length > 0 &&
						(t = i.appendNodes(b, a + 1, n[j], n, f, g && n.open));
					g &&
						(i.makeDOMNodeMainBefore(e, b, n),
				...
				p.ajax({
					...
					success: function(f) {
						if (n == h.getRoot(b)._ver) {
							...
							g && g !== ""
								? ((g = j.apply(b.async.dataFilter, [b.treeId, a, g], g)),
									i.addNodes(b, a, g ? j.clone(g) : [], !!c))
								: i.addNodes(b, a, [], !!c);
				...
				if (b.callback.onNodeCreated || b.view.addDiyDom)
					for (var a = h.getRoot(b); a.createdNodes.length > 0;) {
						...
						b.callback.onNodeCreated &&
							b.treeObj.trigger(e.event.NODECREATED, [b.treeId, c]);
					}
			},
			createNodes: function(b, a, c, d) {
				if (c && c.length != 0) {
					...
					d
						? ((d = k(d, e.id.UL, b)), d.get(0) && d.append(a.join("")))
			...
			expandCollapseNode: function(b, a, c, d, f) {
				...
				if (a) {
					if (g.expandTriggerFlag) {
						var q = f,
							f = function() {
								q && q();
								a.open
									? b.treeObj.trigger(e.event.EXPAND, [b.treeId, a])
									: b.treeObj.trigger(e.event.COLLAPSE, [b.treeId, a]);
							};
					...
					if (
						...
						a.isParent &&
						(!k(a, e.id.UL, b).get(0) ||
							(a[l] && a[l].length > 0 && !k(a[l][0], b).get(0)))
					)
					...
					if (a.open == c) j.apply(f, []);
						...
						a.isParent
							? ((a.open = !a.open),
								a.iconOpen &&
								a.iconClose &&
								o.attr("style", i.makeNodeIcoStyle(b, a)),
								a.open
									? (i.replaceSwitchClass(a, g, e.folder.OPEN),
										i.replaceIcoClass(a, o, e.folder.OPEN),
										d == !1 || b.view.expandSpeed == ""
											? (c.show(), j.apply(f, []))
											: a[l] && a[l].length > 0
												? c.slideDown(b.view.expandSpeed, f)
												: (c.show(), j.apply(f, [])))
									: (i.replaceSwitchClass(a, g, e.folder.CLOSE),
										i.replaceIcoClass(a, o, e.folder.CLOSE),
										d == !1 ||
											b.view.expandSpeed == "" ||
											!(a[l] && a[l].length > 0)
											? (c.hide(), j.apply(f, []))
											: c.slideUp(b.view.expandSpeed, f)))
							: j.apply(f, []);
					}
				} else j.apply(f, []);
			},
			expandCollapseParentNode: function(b, a, c, d, f) {
				a &&
					(a.parentTId
						? (i.expandCollapseNode(b, a, c, d),
							a.parentTId &&
							i.expandCollapseParentNode(b, a.getParentNode(), c, d, f))
			...
			makeDOMNodeIcon: function(b, a, c) {
				...
				b.push(
					...
					e.id.ICON,
					" class='",
					i.makeNodeIcoClass(a, c),
					"' style='",
					i.makeNodeIcoStyle(a, c),
					"'></span><span id='",
					c.tId,
					e.id.SPAN,
					"'>",
			...
			makeNodeIcoClass: function(b, a) {
				var c = ["ico"];
				a.isAjaxing ||
					((c[0] = (a.iconSkin ? a.iconSkin + "_" : "") + c[0]),
						a.isParent
			...
			makeNodeIcoStyle: function(b, a) {
				...
				if (!a.isAjaxing) {
					var d =
						a.isParent && a.iconOpen && a.iconClose
							? a.open
								? a.iconOpen
								: a.iconClose
							: a.icon;
					...
					(b.view.showIcon == !1 ||
						!j.apply(b.view.showIcon, [b.treeId, a], !0)) &&
			...
			makeNodeLineClass: function(b, a) {
				...
				b.view.showLine
					? a.level == 0 && a.isFirstNode && a.isLastNode
						? c.push(e.line.ROOT)
						: a.level == 0 && a.isFirstNode
							? c.push(e.line.ROOTS)
							: a.isLastNode
								? c.push(e.line.BOTTOM)
								: c.push(e.line.CENTER)
					: c.push(e.line.NOLINE);
				a.isParent
					? c.push(a.open ? e.folder.OPEN : e.folder.CLOSE)
					: c.push(e.folder.DOCU);
			...
			removeNode: function(b, a) {
				...
				if (h.getNodeCache(b, a.tId)) {
					...
					if (!b.data.keep.parent && g == 0)
						...
						f == c
							? f[d].length == 1
								? i.replaceSwitchClass(o, l, e.line.ROOT)
								: ((c = k(f[d][0], e.id.SWITCH, b)),
									i.replaceSwitchClass(f[d][0], c, e.line.ROOTS),
									i.replaceSwitchClass(o, l, e.line.BOTTOM))
							: i.replaceSwitchClass(o, l, e.line.BOTTOM);
			...
			setNodeName: function(b, a) {
				...
				d.empty();
				b.view.nameIsHTML
					? d.html(h.getNodeName(b, a))
					: d.text(h.getNodeName(b, a));
			...
			switchNode: function(b, a) {
				a.open || !j.canAsync(b, a)
					...
					: b.async.enable
						? i.asyncNode(b, a) || i.expandCollapseNode(b, a, !a.open)
						: a && i.expandCollapseNode(b, a, !a.open);
			},
		};
	p.fn.zTree = {
		consts: {
		...
		init: function(b, a, c) {
			...
			c = {
				...
				expandNode: function(a, b, c, e, m) {
					...
					if (
						(m = !!m) &&
						b &&
						j.apply(d.callback.beforeExpand, [d.treeId, a], !0) == !1
					)
						return null;
					else if (
						m &&
						!b &&
						j.apply(d.callback.beforeCollapse, [d.treeId, a], !0) == !1
					)
						return null;
					b &&
						a.parentTId &&
						i.expandCollapseParentNode(d, a.getParentNode(), b, !1);
				...
				getNodeByParam: function(a, b, c) {
					return !a
						...
						: h.getNodeByParam(
							d,
							c ? c[d.data.key.children] : h.getNodes(d),
							a,
							b
						);
				},
				...
				getNodeIndex: function(a) {
					...
					for (
						...
						c = a.parentTId ? a.getParentNode() : h.getRoot(d),
						e = 0,
						i = c[b].length;
						e < i;
						e++
				...
				getSelectedNodes: function() {
					for (
						var a = [], b = h.getRoot(d).curSelectedList, c = 0, e = b.length;
						c < e;
						c++
					)
						a.push(b[c]);
				...
				removeNode: function(a, b) {
					a &&
						((b = !!b),
							(b && j.apply(d.callback.beforeRemove, [d.treeId, a], !0) == !1) ||
							(i.removeNode(d, a),
								b &&
								this.setting.treeObj.trigger(e.event.REMOVE, [d.treeId, a])));
				},
				selectNode: function(a, b) {
					if (a && j.uCanDo(d)) {
						b = d.view.selectedMulti && b;
						if (a.parentTId)
							i.expandCollapseParentNode(
								d,
								a.getParentNode(),
								!0,
								!1,
			...
			b[a] && b[a].length > 0
				...
				: d.async.enable && d.async.url && d.async.url !== "" && i.asyncNode(d);
			return c;
		},
	};
	var N = p.fn.zTree,
...
(function(m) {
	...
	m.extend(!0, m.fn.zTree._z, {
		...
		view: {
			checkNodeRelation: function(c, a) {
				...
				if (c.check.chkStyle == b.STYLE) {
						...
						if (c.check.radioType == b.TYPE_ALL) {
							...
							for (d = 0, h = g[i].length; d < h; d++)
								(b = g[i][d]),
									b[l] &&
									b != a &&
									((b[l] = !1), e.setChkClass(c, n(b, j.id.CHECK, c), b));
						}
					...
					a[l] &&
						(!a[i] ||
							a[i].length == 0 ||
							c.check.chkboxType.Y.indexOf("s") > -1) &&
						e.setSonNodeCheckBox(c, a, !0),
						!a[l] &&
						(!a[i] ||
							a[i].length == 0 ||
							c.check.chkboxType.N.indexOf("s") > -1) &&
						e.setSonNodeCheckBox(c, a, !1),
						a[l] &&
						c.check.chkboxType.Y.indexOf("p") > -1 &&
						e.setParentNodeCheckBox(c, a, !0),
						!a[l] &&
				...
				var b = c.data.key.checked,
					...
					i =
						a.chkDisabled === !0
							...
							: a.halfCheck
								? d.PART
								: c.check.chkStyle == h.STYLE
									? a.check_Child_State < 1
										? d.FULL
										: d.PART
									: a[b]
										? a.check_Child_State === 2 || a.check_Child_State === -1
											? d.FULL
											: d.PART
										: a.check_Child_State < 1
											? d.FULL
			...
			repairParentChkClassWithSelf: function(c, a) {
				if (a) {
					...
					a[b] && a[b].length > 0
						? e.repairParentChkClass(c, a[b][0])
						: e.repairParentChkClass(c, a);
				}
			},
			...
			setParentNodeCheckBox: function(c, a, b, d) {
				...
				a.nocheck !== !0 &&
					...
					((a[i] = b),
						...
						a != d &&
						c.treeObj.trigger(j.event.CHECK, [null, c.treeId, a]));
				if (a.parentTId) {
					l = !0;
					if (!b)
						for (var h = a.getParentNode()[h], g = 0, k = h.length; g < k; g++)
							if (
								(h[g].nocheck !== !0 && h[g].chkDisabled !== !0 && h[g][i]) ||
		...
		data: {
			getRadioCheckedList: function(c) {
				for (
					var a = f.getRoot(c).radioCheckedList, b = 0, d = a.length;
					b < d;
					b++
				)
				...
				return {
					...
					half: a.halfCheck
						...
						: c.check.chkStyle == j.radio.STYLE
							...
							: a[b]
								? a.check_Child_State > -1 && a.check_Child_State < 2
									...
									((f =
										g.nocheck === !0 || g.chkDisabled === !0
											...
											: g.halfCheck === !0
												? 1
												: g[d]
													? g.check_Child_State === -1 || g.check_Child_State === 2
														? 2
														: 1
													: g.check_Child_State > 0
														? 1
	...
	f.addInitNode(function(c, a, b, d) {
		if (b) {
			...
			b.chkDisabled =
				!!b.chkDisabled || (c.check.chkDisabledInherit && d && !!d.chkDisabled);
		...
		c.check.enable &&
			(f.makeChkFlag(c, a),
				b.push(
					...
					"' treeNode",
					j.id.CHECK,
					a.nocheck === !0 ? " style='display:none;'" : "",
					"></span>"
				));
	});
	f.addZTreeTools(function(c, a) {
		a.checkNode = function(a, b, c, f) {
			...
			if (
				...
				(a[g] = b),
					...
					(c || this.setting.check.chkStyle === j.radio.STYLE) &&
					e.checkNodeRelation(this.setting, a),
					e.setChkClass(this.setting, b, a),
					e.repairParentChkClassWithSelf(this.setting, a),
					f &&
					this.setting.treeObj.trigger(j.event.CHECK, [
						null,
						this.setting.treeId,
						a,
					]);
		...
		a.getCheckedNodes = function(a) {
			var b = this.setting.data.key.children;
			return f.getTreeCheckedNodes(
				this.setting,
				f.getRoot(this.setting)[b],
				a !== !1
			);
		};
		a.getChangeCheckedNodes = function() {
			var a = this.setting.data.key.children;
			return f.getTreeChangeCheckedNodes(
				this.setting,
				f.getRoot(this.setting)[a]
			);
		};
...
(function(w) {
				...
				function c(b) {
					...
					if (C.dragFlag == 0) {
						...
						for (a = 0, c = l.length; a < c; a++) {
							...
							n = l[a];
							n.isParent && n.open
								? (f.expandCollapseNode(e, n, !n.open),
									(C.dragNodeShowBefore[n.tId] = !0))
						...
						if (l.length > 1) {
							...
							for (a = 0, c = j.length; a < c; a++)
								if (
									(C.dragNodeShowBefore[j[a].tId] !== void 0 &&
										(n && k > -1 && k + 1 !== a && (n = !1),
											i.push(j[a]),
											(k = a)),
										l.length === i.length)
						...
						for (a = 0, c = l.length; a < c; a++)
							(n = l[a]),
								...
								((k = o("<li id='" + n.tId + "_tmp'></li>", e)),
									k.append(o(n, d.id.A, e).clone()),
									k.css("padding", "0"),
									k
										.children("#" + n.tId + d.id.A)
										.removeClass(d.node.CURSELECTED),
									D.append(k),
									a == e.edit.drag.maxShowNodeNum - 1 &&
									((k = o(
										"<li id='" + n.tId + "_moretmp'><a>  ...  </a></li>",
										e
									)),
										D.append(k)));
					...
					if (C.dragFlag == 1) {
						s &&
							...
							u &&
							b.clientX + F.scrollLeft() + 2 >
							w("#" + u + d.id.A, s).offset().left
							? ((n = w("#" + u + d.id.A, s)),
								(b.target = n.length > 0 ? n.get(0) : b.target))
							: s &&
							(s.removeClass(d.node.TMPTARGET_TREE),
								u &&
								w("#" + u + d.id.A, s)
									.removeClass(d.node.TMPTARGET_NODE + "_" + d.move.TYPE_PREV)
									.removeClass(d.node.TMPTARGET_NODE + "_" + I.move.TYPE_NEXT)
									.removeClass(
										d.node.TMPTARGET_NODE + "_" + I.move.TYPE_INNER
									));
						u = s = null;
						J = !1;
						h = e;
						...
						for (var y in n)
							if (
								...
								n[y].edit.enable &&
								n[y].treeId != e.treeId &&
								(b.target.id == n[y].treeId ||
									w(b.target).parents("#" + n[y].treeId).length > 0)
							)
								(J = !0), (h = n[y]);
						y = F.scrollTop();
						k = F.scrollLeft();
						...
						var j = p < e.edit.drag.borderMax && p > e.edit.drag.borderMin,
							...
							p =
								c > e.edit.drag.borderMin &&
								p > e.edit.drag.borderMin &&
								q > e.edit.drag.borderMin &&
								x > e.edit.drag.borderMin,
						...
						if (b.target && g.isChildOrSelf(b.target, h.treeId)) {
							for (
								var E = b.target;
								E && E.tagName && !g.eqs(E.tagName, "li") && E.id != h.treeId;
						...
						if (p && g.isChildOrSelf(b.target, h.treeId)) {
							if (
								!s &&
								(b.target.id == h.treeId || q || x || P || Q) &&
								(J || (!J && n.parentTId))
							)
								s = h.treeObj;
							i
								? h.treeObj.scrollTop(h.treeObj.scrollTop() - 10)
								: j && h.treeObj.scrollTop(h.treeObj.scrollTop() + 10);
							K
								? h.treeObj.scrollLeft(h.treeObj.scrollLeft() - 10)
								: G && h.treeObj.scrollLeft(h.treeObj.scrollLeft() + 10);
							s &&
								s != h.treeObj &&
								s.offset().left < h.treeObj.offset().left &&
								h.treeObj.scrollLeft(
									h.treeObj.scrollLeft() +
									s.offset().left -
						...
						if (s && s.attr("id") != h.treeId) {
							...
							c =
								((b.ctrlKey || b.metaKey) &&
									e.edit.drag.isMove &&
									e.edit.drag.isCopy) ||
								(!e.edit.drag.isMove && e.edit.drag.isCopy);
							a = !!(H && u === H.tId);
							i = !!(R && u === R.tId);
							k = n.parentTId && n.parentTId == u;
							n =
								(c || !i) &&
								g.apply(h.edit.drag.prev, [h.treeId, l, z], !!h.edit.drag.prev);
							a =
								(c || !a) &&
								g.apply(h.edit.drag.next, [h.treeId, l, z], !!h.edit.drag.next);
							G =
								(c || !k) &&
							...
							if (!n && !a && !G) {
								...
								i = z.isLastNode
									? null
									: w("#" + z.getNextNode().tId + d.id.A, s.next());
								j = c.offset().top;
								k = c.offset().left;
								...
								(K == 1 || (y <= K && y >= -0.2)) && n
									...
									: (G == 0 || (y >= G && y <= 1.2)) && a
										? ((a = 1 - B.width()),
											(i =
												i == null || (z.isParent && z.open)
													? j + c.height() - B.height() / 2
													: i.offset().top - B.height() / 2),
											(v = d.move.TYPE_NEXT))
								...
								if (
									...
									(window.zTreeMoveTimer = setTimeout(function() {
										v == d.move.TYPE_INNER &&
											...
											g.apply(h.callback.beforeDragOpen, [h.treeId, z], !0) &&
											(f.switchNode(h, z),
												h.edit.drag.autoExpandTrigger &&
												h.treeObj.trigger(d.event.EXPAND, [h.treeId, z]));
							...
							((v = d.move.TYPE_INNER),
								s &&
									...
									? s.addClass(d.node.TMPTARGET_TREE)
									: (s = null),
								B.css({ display: "none" }),
								window.zTreeMoveTimer)
						)
					...
					if (C.dragFlag != 0) {
						...
						for (a = 0, i = l.length; a < i; a++)
							(j = l[a]),
								...
								!j.open &&
								(f.expandCollapseNode(e, j, !j.open),
						...
						if (s) {
							...
							if (
								g.apply(e.callback.beforeDrop, [h.treeId, l, q, v, p], !0) == !1
							)
								f.selectNodes(x, l);
							else {
								var A = p ? g.clone(l) : l;
								a = function() {
									if (J) {
										...
										if (v == d.move.TYPE_INNER) f.addNodes(h, q, A);
											...
											(f.addNodes(h, q.getParentNode(), A),
												v == d.move.TYPE_PREV)
										)
										...
										(p && f.addNodes(h, q.getParentNode(), A),
											v != d.move.TYPE_NEXT)
									)
										for (a = 0, c = A.length; a < c; a++)
											f.moveNode(h, q, A[a], v, !1);
								...
								v == d.move.TYPE_INNER && g.canAsync(h, q)
									? f.asyncNode(h, q, !1, a)
									: a();
							}
						} else
							f.selectNodes(x, l),
								e.treeObj.trigger(d.event.DROP, [
									b,
									e.treeId,
									l,
									null,
	...
	w.extend(!0, w.fn.zTree._z, {
		...
		view: {
			addEditBtn: function(b, a) {
				if (
					!(a.editNameFlag || o(a, d.id.EDIT, b).length > 0) &&
					g.apply(b.edit.showRenameBtn, [b.treeId, a], b.edit.showRenameBtn)
				) {
					var c = o(a, d.id.A, b),
						r =
							"<span class='" +
							...
							d.id.EDIT +
							"' title='" +
							g.apply(b.edit.renameTitle, [b.treeId, a], b.edit.renameTitle) +
							"' treeNode" +
							d.id.EDIT +
					...
					o(a, d.id.EDIT, b)
						.bind("click", function() {
							if (
								!g.uCanDo(b) ||
								g.apply(b.callback.beforeEditName, [b.treeId, a], !0) == !1
							)
								return !1;
							f.editNode(b, a);
							return !1;
			...
			addRemoveBtn: function(b, a) {
				if (
					!(a.editNameFlag || o(a, d.id.REMOVE, b).length > 0) &&
					g.apply(b.edit.showRemoveBtn, [b.treeId, a], b.edit.showRemoveBtn)
				) {
					var c = o(a, d.id.A, b),
						r =
							"<span class='" +
							...
							d.id.REMOVE +
							"' title='" +
							g.apply(b.edit.removeTitle, [b.treeId, a], b.edit.removeTitle) +
							"' treeNode" +
							d.id.REMOVE +
							" style='display:none;'></span>";
					c.append(r);
					o(a, d.id.REMOVE, b)
						.bind("click", function() {
							if (
								!g.uCanDo(b) ||
								g.apply(b.callback.beforeRemove, [b.treeId, a], !0) == !1
							)
								return !1;
							f.removeNode(b, a);
					...
					k.bind("blur", function() {
						...
						.bind("keydown", function(a) {
							a.keyCode == "13"
								...
								: a.keyCode == "27" && f.cancelCurEditNode(b, null, !0);
						})
						.bind("click", function() {
							return !1;
						})
				...
				if (
					...
					if (r == d.move.TYPE_INNER)
						if (t) c.parentTId = null;
							...
							if (!a.isParent)
								(a.isParent = !0), (a.open = !!a.open), f.setNodeLineIcos(b, a);
							c.parentTId = a.tId;
						}
					var p;
					t
						? (p = t = b.treeObj)
						: (!i && r == d.move.TYPE_INNER
							? f.expandCollapseNode(b, a, !0, !1)
							: i || f.expandCollapseNode(b, a.getParentNode(), !0, !1),
							(t = o(a, b)),
							(p = o(a, d.id.UL, b)),
							t.get(0) &&
							!p.get(0) &&
							((p = []), f.makeUlHtml(b, a, p, ""), t.append(p.join(""))),
							(p = o(a, d.id.UL, b)));
					var q = o(c, b);
					q.get(0)
						? t.get(0) || q.remove()
						: (q = f.appendNodes(b, c.level, [c], null, !1, !0).join(""));
					p.get(0) && r == d.move.TYPE_INNER
						? p.append(q)
						: t.get(0) && r == d.move.TYPE_PREV
							? t.before(q)
							: t.get(0) && r == d.move.TYPE_NEXT && t.after(q);
					var l = -1,
						w = 0,
						x = null,
						t = null,
	...
	m.addInitProxy(function(b) {
		...
		if (g.eqs(b.type, "mouseover")) {
			if (
				(o = g.getMDom(c, a, [{ tagName: "a", attrName: "treeNode" + d.id.A }]))
			)
				(k = g.getNodeMainDom(o).id), (j = "hoverOverNode");
		} else if (g.eqs(b.type, "mouseout"))
			(o = g.getMDom(c, f, [{ tagName: "a", attrName: "treeNode" + d.id.A }])),
				o || ((k = "remove"), (j = "hoverOutNode"));
		else if (
			g.eqs(b.type, "mousedown") &&
			(o = g.getMDom(c, a, [{ tagName: "a", attrName: "treeNode" + d.id.A }]))
		)
	...
	m.addZTreeTools(function(b, a) {
		a.cancelEditName = function(a) {
			m.getRoot(this.setting).curEditNode &&
				f.cancelCurEditNode(this.setting, a ? a : null, !0);
		};
		a.copyNode = function(a, b, k, i) {
			...
			k == d.move.TYPE_INNER
				? ((b = function() {
					...
					g.canAsync(this.setting, a)
						? f.asyncNode(this.setting, a, i, b)
						: b())
				: (f.addNodes(this.setting, a.parentNode, [e], i),
		...
		a.editName = function(a) {
			a &&
				...
				(a.parentTId &&
					f.expandCollapseParentNode(this.setting, a.getParentNode(), !0),
		...
		a.moveNode = function(a, b, k, i) {
			...
			if (
				...
				a &&
				((b.parentTId == a.tId && k == d.move.TYPE_INNER) ||
					o(b, this.setting).find("#" + a.tId).length > 0)
			)
				return null;
			else a || (a = null);
			var e = this;
			g.canAsync(this.setting, a) && k === d.move.TYPE_INNER
				? f.asyncNode(this.setting, a, i, j)
				: j();
			return b;
		};
	...
	g.uCanDo = function(b, a) {
		...
		if (
			a &&
			(g.eqs(a.type, "mouseover") ||
				g.eqs(a.type, "mouseout") ||
				g.eqs(a.type, "mousedown") ||
				g.eqs(a.type, "mouseup"))
		)
			return !0;
